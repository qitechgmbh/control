...
import { PresetsPage } from "@ui/components/preset/PresetsPage";
import { extruder3 } from "@ui/machines/properties";
import { useExtruder3 } from "./useExtruder";
import { z } from "zod";
import {
  PresetPreviewEntries,
  previewSeparator,
} from "@ui/components/preset/PresetPreviewTable";
import { Preset } from "@ui/lib/preset/preset";
...
const previewEntries: PresetPreviewEntries<Extruder3> = [
  ...
  previewSeparator,
  {
    ...
    renderValue: (data: Extruder3) => data.inverterRegulation,
  },
  {
    name: "Target Inverter RPM",
    unit: "rpm",
    renderValue: (data: Extruder3) => data.targetInverterRpm?.toFixed(2),
  },
  {
    name: "Target Inverter Pressure",
    unit: "bar",
    renderValue: (data: Extruder3) => data.targetInverterPressure?.toFixed(1),
  },
  {
    name: "Inverter Direction",
    renderValue: (data: Extruder3) => data.inverterRotationDirection,
  },
  previewSeparator,
  {
    name: "Pressure Limit",
    unit: "bar",
    renderValue: (data: Extruder3) => data.pressureLimit?.toFixed(1),
  },
  {
  ...
  {
    name: "PID Pressue Kp",
    renderValue: (data: Extruder3) => data.pidPressureKp?.toFixed(4),
  },
  {
    name: "PID Pressue Ki",
    renderValue: (data: Extruder3) => data.pidPressureKi?.toFixed(4),
  },
  {
    name: "PID Pressue Kd",
