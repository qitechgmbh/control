...
import type { EnvironmentInfo, NixOSGeneration } from "@ui/bridge";
...
import { CollapsibleContent } from "@ui/components/ui/collapsible";
import { useUpdateStore } from "@ui/stores/updateStore";
import { useGithubSourceStore } from "@ui/stores/githubSourceStore";
...
export function ChooseVersionPage() {
  ...
  useEffectAsync(async () => {
    const _environmentInfo = await getBridge().environment.getInfo();
  ...
  useEffectAsync(async () => {
    if (!getBridge().nixos.isNixOSAvailable) {
      setNixosGenerations([]);
      return;
    }

    try {
      const generations = await getBridge().nixos.listGenerations();
  ...
  const handleSetGeneration = async (generationId: string) => {
    ...
    try {
      await getBridge().nixos.setGeneration(generationId);
  ...
  const handleDeleteGeneration = async (generationId: string) => {
    ...
    setGenerationActionLoading(generationId);
    try {
      await getBridge().nixos.deleteGeneration(generationId);
  ...
  const handleDeleteAllOldGeneration = async () => {
    ...
    try {
      await getBridge().nixos.deleteAllOldGenerations();
  ...
  return (
    <Page>
      ...
      {!getBridge().nixos.isNixOSAvailable && (
        <div className="flex items-center justify-center">
      ...
      {nixosGenerations === undefined && getBridge().nixos.isNixOSAvailable && (
        <LoadingSpinner />
      )}
