//! Modbus RTU framing for serial links that are driven synchronously from the act loop
//! (i.e. an EL6021 terminal reached through [`SerialInterfaceDevice`]).
//!
//! This is deliberately *not* `qitech_lib::modbus`, which is an async `tokio-modbus` client over a
//! host serial port and cannot drive an EtherCAT-attached terminal.
//!
...
///
/// Exercised by the unit tests below; the framing path uses `crc` directly.
#[allow(dead_code)]
...
fn extract_crc(raw_data: &[u8]) -> Result<u16, Error> {
    if raw_data.len() < 5 {
