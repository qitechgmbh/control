    ...
    match interfaces{
        Ok(interfaces) => {
            for interface in interfaces {
                ...
                let res = qitech_lib::ethercat_hal::interface_discovery::test_interface(&interface.name);
                match res {
                    Ok(_) => {
...
fn main_logic() {
    ...
    println!("Test: {}", args.get(1).unwrap_or(&"Failed".to_owned()));
