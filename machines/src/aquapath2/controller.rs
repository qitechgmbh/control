...
impl Controller {
    ...
    /// Returns a flow rate value based on whether the UFM sensor detects any water flowing.
    /// Returns `min_flow_for_thermal` when flow is detected, `0` otherwise.
    pub fn get_flow(&mut self, now: Instant) -> VolumeRate {
        match self.flow_sensor.tick(now) {
            Ok(data) => {
                if data.flow_lph > 0.0 {
                    self.config.min_flow_for_thermal
                } else {
                    VolumeRate::new::<liter_per_minute>(0.0)
                }
            }
            Err(_) => VolumeRate::new::<liter_per_minute>(0.0),
        }
    }
