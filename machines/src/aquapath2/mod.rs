...
use crate::{
    MACHINE_AQUAPATH_V2, VENDOR_QITECH,
    aquapath2::{
        api::{
            AquaPathV2Events, AquaPathV2Namespace, CoolingModeState, CoolingModeStates, FanState,
...
impl AquaPathV2 {
    ...
    pub fn emit_live_values(&mut self) {
        ...
        self.namespace.emit(AquaPathV2Events::LiveValues(event));
    }
    ...
    pub fn emit_state(&mut self) {
        ...
        self.namespace.emit(AquaPathV2Events::State(event));
    }
    ...
    pub fn emit_notice(&mut self, title: impl Into<String>, message: impl Into<String>) {
        ...
        .build();
        self.namespace.emit(AquaPathV2Events::Notice(event));
    }
    ...
    fn mode_allows_standby_only_config(&self) -> bool {
        self.mode == AquaPathV2Mode::Standby
    }
}

impl std::fmt::Display for AquaPathV2 {
...
impl AquaPathV2 {
    ...
    fn switch_to_standby(&mut self) {
        match self.mode {
            AquaPathV2Mode::Standby => (),
            AquaPathV2Mode::Auto => self.turn_off_all(),
        };
        self.mode = AquaPathV2Mode::Standby;
    }
    ...
    fn switch_to_auto(&mut self) {
        match self.mode {
            AquaPathV2Mode::Auto => (),
            AquaPathV2Mode::Standby => self.turn_on_all(),
        }
        self.mode = AquaPathV2Mode::Auto;
    }

    fn switch_mode(&mut self, mode: AquaPathV2Mode) {
        if self.mode == mode {
            return;
        }
        match mode {
            AquaPathV2Mode::Standby => self.switch_to_standby(),
            AquaPathV2Mode::Auto => self.switch_to_auto(),
        }
    }
}

impl AquaPathV2 {
    fn set_mode_state(&mut self, mode: AquaPathV2Mode) {
...
impl AquaPathV2 {
    ...
    fn set_target_temperature(&mut self, temperature: f64, side: AquaPathSideType) {
    ...
    fn set_should_pump(&mut self, should_pump: bool, side: AquaPathSideType) {
        match side {
...
impl AquaPathV2 {
    fn set_max_revolutions(&mut self, revolutions: f64, side: AquaPathSideType) {
...
impl AquaPathV2 {
    ...
    fn set_heating_tolerance(&mut self, tolerance: f64, side: AquaPathSideType) {
        match side {
    ...
    fn set_cooling_tolerance(&mut self, tolerance: f64, side: AquaPathSideType) {
        match side {
            AquaPathSideType::Back => {
                let fallback = self
                    .back_controller
    ...
    fn set_pump_cooldown_min_temperature(&mut self, temperature: f64, side: AquaPathSideType) {
        ...
        let temp = ThermodynamicTemperature::new::<degree_celsius>(value);
        match side {
            AquaPathSideType::Back => self.back_controller.set_pump_cooldown_min_temperature(temp),
            AquaPathSideType::Front => self
                .front_controller
                .set_pump_cooldown_min_temperature(temp),
        }
        self.emit_state();
    }
}
