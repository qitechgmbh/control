...
use super::super::tension::{TensionArm, normalize_angle_deg};
...
#[derive(Debug)]
pub struct Stepper5TensionController {
...
impl Stepper5TensionController {
    ...
    fn speed_raw(&self, master_speed: Velocity, tension_arm: &TensionArm) -> Velocity {
        ...
        let current_angle =
            Angle::new::<degree>(normalize_angle_deg(tension_arm.get_angle().get::<degree>()))
                .abs();
...
mod tests {
    ...
    fn test_addon_tension_controller_defaults() {
        let controller =
            Stepper5TensionController::new(Angle::new::<degree>(55.0), Angle::new::<degree>(35.0));
    ...
    fn test_addon_tension_controller_limits() {
        ...
        let mut tension_arm = TensionArm::new(analog_input_dummy.analog_input());
        tension_arm.zero(); // zero at 0° so get_angle() reads relative to 0
    ...
    #[test]
    fn test_stepper_5_over_target_speeds_up_and_under_target_slows_down() {
        let mut controller =
            Stepper5TensionController::new(Angle::new::<degree>(55.0), Angle::new::<degree>(35.0));
        controller.set_enabled(true);

        let master_speed = Velocity::new::<meter_per_second>(1.0);
        let (mut analog_input_dummy, mut tension_arm) = create_tension_arm();
        tension_arm.zero();
    ...
    fn test_zero_sensitivity_returns_master_speed() {
        let mut controller =
            Stepper5TensionController::new(Angle::new::<degree>(55.0), Angle::new::<degree>(0.0));
        controller.set_enabled(true);
    ...
    fn test_wraparound_zero_angle_stays_slow() {
        ...
        // 359° normalizes to -1°, which is ≤ 0° → motor stops completely.
        ...
        assert_eq!(
            out,
            Velocity::ZERO,
            "wraparound angle near 360° (→ -1° normalized) must stop the motor"
        );
    }
