mod gluetex_imports {
    pub use super::super::controllers::line::{GearRatio, PullerRegulationMode};
    ...
    fn api_mutate(&mut self, request_body: Value) -> Result<(), anyhow::Error> {
        ...
        match mutation {
            ...
            Mutation::SetAddonMotor3Enabled(enabled) => {
                self.stepper_3_controller.set_enabled(enabled);
                self.emit_state();
            }
            Mutation::SetAddonMotor3Forward(forward) => {
                self.stepper_3_controller.set_forward(forward);
                self.emit_state();
            }
            Mutation::SetAddonMotor4Enabled(enabled) => {
                self.stepper_4_controller.set_enabled(enabled);
                self.emit_state();
            }
            Mutation::SetAddonMotor4Forward(forward) => {
                self.stepper_4_controller.set_forward(forward);
                self.emit_state();
            }
            Mutation::SetAddonMotor3MasterRatio(ratio) => {
                self.stepper_3_controller.set_master_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor3SlaveRatio(ratio) => {
                self.stepper_3_controller.set_slave_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor4MasterRatio(ratio) => {
                self.stepper_4_controller.set_master_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor4SlaveRatio(ratio) => {
                self.stepper_4_controller.set_slave_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor5Enabled(enabled) => {
                self.stepper_5_controller.set_enabled(enabled);
                self.emit_state();
            }
            Mutation::SetAddonMotor5Forward(forward) => {
                self.stepper_5_controller.set_forward(forward);
                self.emit_state();
            }
            Mutation::SetAddonMotor5MasterRatio(ratio) => {
                self.stepper_5_controller.set_master_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor5SlaveRatio(ratio) => {
                self.stepper_5_controller.set_slave_ratio(ratio);
                self.emit_state();
            }
            Mutation::SetAddonMotor5TensionEnabled(enabled) => {
                self.stepper_5_tension_controller.set_enabled(enabled);
                self.emit_state();
            }
            Mutation::SetAddonMotor5TensionTargetAngle(angle_deg) => {
                self.stepper_5_tension_controller
            ...
            Mutation::SetAddonMotor3Konturlaenge(length_mm) => {
                self.stepper_3_controller.set_konturlaenge_mm(length_mm);
                self.emit_state();
            }
            Mutation::SetAddonMotor3Pause(pause_mm) => {
                self.stepper_3_controller.set_pause_mm(pause_mm);
                self.emit_state();
            }
            Mutation::SetAddonMotor3DecelerationDistance(distance_mm) => {
                self.stepper_3_controller
                    .set_deceleration_distance_mm(distance_mm);
                self.emit_state();
            }
            Mutation::HomeAddonMotor3 => {
                self.stepper_3_controller.start_manual_homing();
                self.emit_state();
            }
