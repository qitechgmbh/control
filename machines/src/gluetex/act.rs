...
impl MachineAct for Gluetex {
    fn act(&mut self, now: Instant) {
        ...
        self.sync_stepper_3_speed(now);
        self.sync_stepper_4_speed(now);
        self.sync_stepper_5_speed(now);
        ...
        if super::safety::run_tension_and_voltage_checks(self) {
            self.emit_state();
        }
        ...
        let autotuning_active = self.heaters.any_autotuning();
