use crate::wago_winder::{
    filament_tension::FilamentTensionCalculator, puller_speed_controller::PullerSpeedController,
};
...
impl MinMaxSpoolSpeedController {
    ...
    fn speed_raw(&mut self, _t: Instant, tension_arm: &TensionArm) -> AngularVelocity {
        ...
        let tension_arm_angle_deg = tension_arm_angle.get::<degree>();

        if !(20.0..=90.0).contains(&tension_arm_angle_deg) {
            return min_speed;
        }

        let filament_tension = self.filament_calc.calc_filament_tension(tension_arm_angle);
