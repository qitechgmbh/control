...
use crate::serial::devices::us_3202510::{
    register::{HoldingRegister, InputRegister},
    transport::{CustomTransport, EntryContext},
};
...
impl US3202510 {
    ...
    fn queue_request(&mut self, request: Request) {
        ...
        match self
            .interface
            .queue_request(data.type_id, data.payload, data.priority)
        {
            Ok(_) => {}
            Err(x) => match x {
                modbus::QueueItemError::QueueFull => {
                    tracing::error!("QueueFull!");
                }
                modbus::QueueItemError::DuplicateItem => {
                    tracing::error!("DuplicateItem!");
                }
            },
        }
    }
    ...
    pub fn set_running(&mut self, running: bool) {
        if self.config.running == running {
            return;
        }
        ...
        match running {
            ...
            false => {
                self.queue_request(Request::SetRotationState(2));
            }
        }
    ...
    pub fn set_acceleration_level(&mut self, acceleration_level: u16) {
        ...
        self.queue_request(Request::SetAccelerationLevel(
            self.config.acceleration_level,
        ));
    }
    ...
    pub fn set_deceleration_level(&mut self, deceleration_level: u16) {
        ...
        self.queue_request(Request::SetDecelerationLevel(
            self.config.deceleration_level,
        ));
    }
    ...
    fn handle_result(&mut self, response: RequestResult) {
        match response {
            RequestResult::ReadHoldingRegisters(data) => {
                let frequency_target: u16 = *data.result.get(0).unwrap_or(&0);

                let _run_command: u16 = *data.result.get(1).unwrap_or(&0);

                let acceleration_level: u16 = *data.result.get(2).unwrap_or(&0);

                let deceleration_level: u16 = *data.result.get(3).unwrap_or(&0);
            ...
            RequestResult::ReadInputRegisters(data) => {
                let frequency: u16 = *data
                    .result
                    .get(InputRegister::CurrentFrequency as usize)
                    .unwrap_or(&0);

                let voltage: u16 = *data
                    .result
                    .get(InputRegister::BusVoltage as usize)
                    .unwrap_or(&0);

                let current: u16 = *data
                    .result
                    .get(InputRegister::LineCurrent as usize)
                    .unwrap_or(&0);

                let temperature: u16 = *data
                    .result
                    .get(InputRegister::DriveTemperature as usize)
                    .unwrap_or(&0);

                let operation_status: u16 = *data
                    .result
                    .get(InputRegister::SystemStatus as usize)
                    .unwrap_or(&0);
                ...
                self.status = Some(Status {
                    ...
                    temperature: units::ThermodynamicTemperature::new::<kelvin>(
                        (temperature as f64),
                    ),
                    operation_status: if operation_status == 0 {
                        OperationStatus::Idle
                    } else if operation_status == 1 {
                        OperationStatus::Running
                    } else {
                        OperationStatus::Fault
                    },
                });
            }
