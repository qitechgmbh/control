...
pub struct StateEvent {
    ...
    pub connected_machine_state: ConnectedMachineData,
    /// sinewave state
...
impl MachineApi for BufferV1 {
    fn api_mutate(&mut self, request_body: Value) -> Result<(), anyhow::Error> {
        println!("api_mutate called with: {:?}", request_body);
        let mutation: Mutation = serde_json::from_value(request_body)?;
        println!("Mutation deserialized: {:?}", mutation);
        match mutation {
            Mutation::SetBufferMode(mode) => self.set_mode_state(mode),
            Mutation::SetConnectedMachine(machine) => self.set_machine_state(machine),
