...
use control_core::{machines::connection::MachineSlot};
...
impl< U: Any + 'static> Downcast<Arc<Mutex<MachineSlot<LaserMachine>>>> for Arc<Mutex<U>> {
    fn downcast(&self) -> Result<Arc<Mutex<MachineSlot<LaserMachine>>>, anyhow::Error> {
...

        {
            ...
            if !lock.is::<MachineSlot<LaserMachine>>() {
                // Transmute the Arc to the desired type
                return Err(anyhow!("[{}::downcast] Downcast failed", module_path!()));
            }
        }
        unsafe {
            let arc = Arc::from_raw(Arc::into_raw(cloned) as *const Mutex<MachineSlot<LaserMachine>>);
