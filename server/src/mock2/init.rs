...
use {
    ...
    crate::serial::devices::mock2::Mock2SerialDevice,
    control_core::{
...
#[cfg(feature = "mock-machine")]
pub fn init_mock2(app_state: Arc<AppState>) -> Result<(), anyhow::Error> {
    ...
    return smol::block_on(async {
        ...
        let serial_params = SerialDeviceNewParams {
            path: "/dev/mock-serial2".to_string(),
            device_thread_panic_tx,
        };

        // Create the mock serial device
        match Mock2SerialDevice::new_serial(&serial_params) {
    ...
    tracing::info!("Mock2 machines initialized successfully");
    Ok(())
}
