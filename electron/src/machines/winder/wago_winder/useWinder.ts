...
import { VENDOR_QITECH, wagoWinder } from "@/machines/properties";
import { wagoWinderSerialRoute } from "@/routes/routes";
import { z } from "zod";
import {
  SpoolRegulationMode,
  StateEvent,
...
export function useWinder2() {
  const { serial: serialString } = wagoWinderSerialRoute.useParams();
  ...
  const machineIdentification: MachineIdentificationUnique = useMemo(() => {
    ...
    return {
      machine_identification: wagoWinder.machine_identification,
      serial,
    };
  }, [serialString]);
