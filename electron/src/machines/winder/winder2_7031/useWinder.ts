...
import { VENDOR_QITECH, winder2_spool_7031 } from "@/machines/properties";
import { winder2_7031SerialRoute } from "@/routes/routes";
import { z } from "zod";
import {
  SpoolRegulationMode,
  StateEvent,
...
export function useWinder2() {
  const { serial: serialString } = winder2_7031SerialRoute.useParams();
  ...
  const machineIdentification: MachineIdentificationUnique = useMemo(() => {
    ...
    return {
      machine_identification: winder2_spool_7031.machine_identification,
      serial,
    };
  }, [serialString]);
