...
import { EXTRUDER_TARGET_KEYS } from "../extruder2/extruder2Namespace";
...
// Target time series tracker with change detection (reuses shared extruder target keys)
const extruder3TargetTracker =
    ...
    try {
      if (eventName === "StateEvent") {
        ...
        updateStore((state) => {
          ...
          const targetUpdates = extruder3TargetTracker.updateTargets(
            state,
            targetValues,
            timestamp,
          );
  ...
  (): StoreApi<Extruder3NamespaceStore> =>
    create<Extruder3NamespaceStore>(() => {
      return {
        ...
        ...extruder3TargetTracker.initialSeries,
      };
    });
