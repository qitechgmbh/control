...
const previewEntries: PresetPreviewEntry<Extruder2>[] = [
];

export function Extruder2PresetsPage() {
  const { state, defaultState, setFrontHeatingTemperature, setMiddleHeatingTemperature, setBackHeatingTemperature, setNozzleHeatingTemperature, setInverterTargetRpm, setInverterTargetPressure, setInverterRegulation, setInverterRotationDirection, setExtruderPressureLimit, setExtruderPressureLimitEnabled, setPressurePidKd, setPressurePidKi, setPressurePidKp } = useExtruder2();
  ...
  const toPresetData = (s?: typeof state): Extruder2PresetData => ({
    ...
    inverterRotationDirection: s?.rotation_state?.forward ? "Forward" : "Backward",
  ...
  const applyPreset = (preset: Preset<Extruder2>) => {
      setFrontHeatingTemperature(preset.data.targetFrontHeatingTemperature ?? 150.0);
      setMiddleHeatingTemperature(preset.data.targetMiddleHeatingTemperature ?? 150.0);
      setBackHeatingTemperature(preset.data.targetBackHeatingTemperature ?? 150.0);
      setNozzleHeatingTemperature(preset.data.targetNozzleHeatingTemperature ?? 150.0);
      ...
      setInverterRotationDirection(preset.data.inverterRotationDirection === "Forward");
      ...
      setPressurePidKp(preset.data.pidPressureKp ?? 0.160);
      setPressurePidKi(preset.data.pidPressureKi ?? 0.000);
