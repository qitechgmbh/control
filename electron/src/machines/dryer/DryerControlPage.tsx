...
import { MaterialStoreState } from "./materialStore";
...
/// Shared by both the V1 and Smart dryer routes - the Smart-only "Smart" card at the
/// bottom is gated on `v.is_smart`, reported directly by the connected hardware, rather
/// than on which route rendered this component.
export function DryerControlPage({
  machineIdentification,
  materialStore,
}: {
  machineIdentification: MachineIdentificationUnique;
  materialStore: MaterialStoreState;
}) {
...
function FlowRateCard({
  ...
  const { throughput, selectedAbbrev, setThroughput } = materialStore;
  const { request: sendMutation } = useMachineMutate(z.any());
