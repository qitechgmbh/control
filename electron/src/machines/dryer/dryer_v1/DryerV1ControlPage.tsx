...
export function DryerV1ControlPage() {
  ...
  }, [selectedMaterialAbbrev, materialPresets, setTargetTempOptimistic]);
  ...
  return (
    <Page>
      <ControlGrid columns={3}>
        ...
        {v?.is_smart && (
          <ControlCard title="Smart">
            <div className="text-sm text-gray-400">
              Firmware Version:{" "}
              <span className="font-mono font-semibold text-gray-700">
                {`${v.smart_data.sw_major}.${v.smart_data.sw_middle}.${v.smart_data.sw_minor}`}
              </span>
            </div>
            <Label label="Timer Program">
              <SelectionGroupBoolean
                value={v.smart_data.timer_enabled}
...
function MaterialListeCard({
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return materialPresets;
    return materialPresets.filter(
      (p) =>
        p.abbrev.toLowerCase().includes(q) || p.name.toLowerCase().includes(q),
    );
  }, [search, materialPresets]);

  const favPresets = materialPresets.filter((p) =>
    favorites.includes(p.abbrev),
  );
  const recentPresets = recentlyUsed
    .map((a) => materialPresets.find((p) => p.abbrev === a))
    .filter(Boolean) as typeof materialPresets;

  const selectedPreset = selectedAbbrev
    ? materialPresets.find((p) => p.abbrev === selectedAbbrev)
    : null;
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
          <span>
            ...
            <b className="text-gray-800">
              {formatRange(
                selectedPreset.temp_min,
                selectedPreset.temp_max,
                "°C",
              )}
            </b>
          </span>
          <span>
            ...
            <b className="text-gray-800">
              {formatRange(
                selectedPreset.drying_time_min,
                selectedPreset.drying_time_max,
                " h",
              )}
            </b>
          </span>
