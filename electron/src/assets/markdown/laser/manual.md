...
**Note:** We advice not to enable that, since the tolerance settings on the laser may differ from the tolerance configured in our software.

Configuration steps:
On the Laser Screen go to:
Menu -> set alarm -> set max input -> set min input
and confirm each step with the OP button.

The Alarm will emit a tone if the measured diameter is below or above the set limits.
...
Configuration steps:
On the Laser Screen go to:
Menu -> calibrate -> (Insert the first (4 mm) rod) adjust the diameter on the screen using the arrow keys -> (Insert the second (8 mm) rod) adjust the the diamater on the screen using the arrow keys
and confirm each step with the op button.
