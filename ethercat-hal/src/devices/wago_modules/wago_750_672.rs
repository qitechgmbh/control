...
impl EthercatDevice for Wago750_672 {
    fn input(
        ...
        else if self.state == 3 && (self.s1() & 0x2) != 0
        {
            ...
            // set M_SpeedControl
            self.write_control_bits(1 + 2 + 8 + 16, 0, 0);
            self.state = 4;
        }

        // await ack
        else if self.state == 4 && (self.s1() & 0x18) != 0
        {
            tracing::warn!("State 4: Set Start");
            // send start signal
            self.write_control_bits(1 + 2 + 4 + 8 + 16, 0, 0);
            self.state = 5;
        }
    ...
    fn output(
        ...
        if self.state != 99 {
            tracing::error!("C1: {:08b}", self.rxpdo.c1);
            tracing::info!("S1: {:08b}", self.txpdo.s1);
        }
