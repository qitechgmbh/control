...
impl EL30XXChannelConfiguration {
    pub async fn write_channel_config<'a>(
        ...
        device
            ...
            .await?;
        device.sdo_write(base_index, 0x05, self.siemens_bits).await?;
        device.sdo_write(base_index, 0x06, self.enable_filter).await?;
        device.sdo_write(base_index, 0x07, self.enable_limit_1).await?;
        device.sdo_write(base_index, 0x08, self.enable_limit_2).await?;
        device
        ...
        device
            ...
            .await?;
        device.sdo_write(base_index, 0x0E, self.swap_limit_bits).await?;
        device
            .sdo_write(base_index, 0x11, self.user_scale_offset)
            .await?;
        device.sdo_write(base_index, 0x12, self.user_scale_gain).await?;
