...
impl StepperVelocityWago750672 {
    ...
    pub fn set_velocity_register(&mut self, velocity: i16) {
        ...
        let command_not_accepted = (velocity == 0 && dev.txpdo.actual_velocity != 0)
            || (velocity != 0 && dev.txpdo.actual_velocity == 0);
        if self.enabled && (velocity_changed || command_not_accepted) {
