...
pub fn convert_raw_diagnosis_bytes(message : [u8; DIAG_MESSAGE_LENGTH]) -> Result<SubdeviceDiagnosisEntry,anyhow::Error> {
	...
	let diag_code = u32::from_be_bytes(diag_code_bytes);

	let flags_code_bytes = [message[4],message[5]];
	let flags = u16::from_be_bytes(flags_code_bytes);

	let text_id_bytes = [message[6],message[7]];
	let text_id = u16::from_be_bytes(text_id_bytes);

	let timestamp_bytes = [message[8],message[9],message[10],message[11],message[12],message[13],message[14],message[15]];
	...
	// 0x1005 Apparently Means p1 variable is u8
	...
	if p1 == 0 {
		return Err(  anyhow::anyhow!("convert_raw_diagnosis_bytes: Datatype_p1 is unknown {:?}",data_type_p1));
	}

	// flags_p2 is packed, whatever thats supposed to mean
	let flags_p2_bytes = [message[19],message[20]];
	let flags_p2 = u16::from_be_bytes(flags_p2_bytes);
	let p2 = [message[21],message[22],message[23],message[24],message[25]];
	return Ok(SubdeviceDiagnosisEntry { diag_code, flags, text_id, timestamp, data_type_p1, p1, flags_p2, p2 });
}
...
pub async fn get_most_recent_diagnosis_message(device : &SubDeviceRef<'_,&SubDevice>) -> Option<String>
{
	...
	let new_message_available = match new_message_available {
    	...
    	Err(e) => 
    	{
			tracing::error!("get_most_recent_diagnosis_message: Failed to read new_message_available {:?}",e);
			return None;
    	},
	};	
	...
	let newest_message_index = device.sdo_read::<u8>(DIAGNOSIS_HISTORY_INDEX, NEWEST_MESSAGE).await;
	let newest_message_index = match newest_message_index {
    	Ok(m) => m,
    	Err(e) => 
    	{
			tracing::error!("get_most_recent_diagnosis_message: Failed to read newest_message_index {:?}",e);
			return None;
    	},
	};	

	tracing::info!("get_most_recent_diagnosis_message: Reading newest message at index: {}",newest_message_index);
	let res = device.sdo_read::<[u8; DIAG_MESSAGE_LENGTH]>(DIAGNOSIS_HISTORY_INDEX, newest_message_index).await;
	...
	let message = match message {
    	Ok(msg) => msg,
    	Err(e) => {
			tracing::error!("get_most_recent_diagnosis_message: Failed to convert Diagnosis Message bytes to struct {:?}",e);
			return None;
    	},
	};

	tracing::info!("get_most_recent_diagnosis_message: {}",message);
