...
impl From<ureq::Error> for ApiError {
    fn from(err: ureq::Error) -> Self { ApiError::UreqError(err) }
}

impl From<std::io::Error> for ApiError {
    fn from(err: std::io::Error) -> Self { ApiError::IoError(err) }
}

impl From<serde_json::Error> for ApiError {
    fn from(err: serde_json::Error) -> Self { ApiError::JsonError(err) }
}
...
pub struct Workorders { pub value: Vec<Workorder>, }
#[derive(Debug, Deserialize)]
pub struct WorkOrderLines { pub value: Vec<WorkOrderLine>, }
#[derive(Debug, Deserialize)]
pub struct WorkOrderLine {
    #[serde(rename = "DocEntry")] pub doc_entry: u32,
    #[serde(rename = "LineNumber")] pub line_number: u32,
    #[serde(rename = "LineNumber2")] pub line_number2: u32,
    #[serde(rename = "SortId")] pub sort_id: u32,
    #[serde(rename = "Position")] pub position: String,
    #[serde(rename = "Barcode")] pub barcode: String,
    #[serde(rename = "ItemCode")] pub item_code: String,
    #[serde(rename = "ItemName")] pub item_name: String,
    #[serde(rename = "QuantityTotalWhsUnit")] pub quantity_total_whs_unit: f32,
}
#[derive(Debug, Deserialize)]
pub struct Workorder {
    #[serde(rename = "DocEntry")] pub doc_entry: u32,
    #[serde(rename = "DocNum")] pub doc_num: String,
    #[serde(rename = "DeliveryDate")] pub delivery_date: String,
    #[serde(rename = "CardCode")] pub card_code: String,
    #[serde(rename = "CardName")] pub card_name: String,
    #[serde(rename = "Lock")] pub lock: u32, 
    #[serde(rename = "Closed")] pub closed: u32, 
    #[serde(rename = "PriorityCode")] pub priority_code: String, 
    #[serde(rename = "StartDate")] pub start_date: String, 
    #[serde(rename = "EndDate")] pub end_date: String, 
    #[serde(rename = "ItemCode")] pub item_code: String,
    #[serde(rename = "PlannedTime")] pub planned_time: f64,
    #[serde(rename = "WorkTime")] pub work_time: f64,
    #[serde(rename = "ReservedTime")] pub reserved_time: f64,
    #[serde(rename = "ProductionTypeId")] pub production_type_id: String,
    #[serde(rename = "Locked")] pub locked: bool, 
    #[serde(rename = "ApsStatus")] pub aps_status: bool, 
}
#[derive(Debug,Deserialize,Serialize)]
pub struct LoginResponse {
    pub ret_code : u32,
    pub ret_text : String,
    #[serde(rename = "beas-sessionid")] pub beas_session_id : String,
}

pub fn article_weights() -> HashMap<&'static str, f64> {
    let mut m = HashMap::new();
    m.insert("ZURO-20160", 9.85); m.insert("ZURO-20161", 11.00); m.insert("ZURO-20162", 12.25); m.insert("ZURO-20163", 14.75); m.insert("ZURO-20164", 12.45); m.insert("ZURO-20165", 12.76); m.insert("ZURO-20166", 10.82); m.insert("ZURO-20167", 12.20); m.insert("ZURO-20168", 12.70); m.insert("ZURO-20169", 13.50); m.insert("ZURO-20170", 9.70); m.insert("ZURO-20171", 10.80); m.insert("ZURO-20183", 11.80); m.insert("ZURO-20188", 12.20); m.insert("ZURO-20190", 12.25); m.insert("ZURO-20200", 9.85); m.insert("ZURO-20209", 11.85); m.insert("ZURO-20210", 12.18); m
}
...
fn try_login(server_root: &str, pw : &str) -> Result<LoginResponse, ApiError> {
    ...
    let response: LoginResponse = ureq::post(&res).send_json(ureq::json!({ "ServicePwd": pw })).map_err(|e| ApiError::UreqError(e))?.into_json().map_err(|e| ApiError::IoError(e))?;
    Ok(response)
}

fn get_work_orders(server_root: &str, session_id : &str) -> Result<Workorders, ApiError> {
    let res = format!("{}{}",server_root,"Workorder?$select=DocEntry,DocNum,DeliveryDate,CardCode,CardName,Lock,Closed,PriorityCode,StartDate,EndDate,ItemCode,PlannedTime,WorkTime,ReservedTime,ProductionTypeId,Locked,ApsStatus&$filter=ApsStatus eq true and Closed eq 0&$orderby=StartDate asc");
    println!("trying get workorders to: {}",res);
    let response = ureq::get(&res).set("Cookie",session_id).call().map_err(|e| ApiError::UreqError(e))?;
    let body = response.into_string().map_err(|e| ApiError::IoError(e))?;
    let parsed: Workorders = serde_json::from_str(&body)?;
    Ok(parsed)
}

fn get_work_orders_bom(server_root: &str, session_id : &str, doc_entry : u32) -> Result<WorkOrderLines, ApiError> {
    let get_route_and_params = format!("WorkorderBom?$filter=DocEntry eq {}&$select=DocEntry,LineNumber,LineNumber2,SortId,Position,Barcode,ItemCode,ItemName,QuantityTotalWhsUnit",doc_entry);
...
fn get_weighted_item(item : &WorkOrderLine, weight_map : &HashMap<&'static str, f64>) -> Result<WeightedItem, ApiError> {
    let code : &str = &item.item_code;
    let weight = weight_map.get(code).ok_or_else(|| ApiError::LogicError(format!("Missing weight for item code: {}", code)))?;
    let weighted_item = WeightedItem{ code: item.item_code.clone(), name: item.item_name.clone(), weight: *weight as f32, quantity: item.quantity_total_whs_unit as u32 };
    Ok(weighted_item)
}

fn get_newest_order(work_orders : &Workorders) -> Result<&Workorder, ApiError> {
    work_orders.value.first().ok_or_else(|| ApiError::LogicError("Workorders list is empty.".to_string()))
}
...
pub fn get_newest_weighted_item(server_root: &str, beas_session_id : &str ) -> Result<WeightedItem, ApiError> {
    ...
    let found: Vec<_> = bom.value.iter().filter(|item| item.item_code.contains("ZURO-")).collect();
    let first_item = found.first().ok_or_else(|| ApiError::LogicError("No ZURO- item found in BOM.".to_string()))?;
...
impl From<ChannelError> for WorkerError { fn from(err: ChannelError) -> Self { WorkerError::Channel(err) } }
...
impl From<RecvError> for CustomChannelError { fn from(err: RecvError) -> Self { CustomChannelError::Receive(format!("Blocking receive error: {}", err)) } }
impl From<SendError<WeightedItem>> for CustomChannelError { fn from(err: SendError<WeightedItem>) -> Self { CustomChannelError::Send(format!("Data send error: {}", err)) } }
impl From<SendError<ApiConfig>> for CustomChannelError { fn from(err: SendError<ApiConfig>) -> Self { CustomChannelError::Send(format!("Config send error: {}", err)) } }

impl From<smol::channel::TryRecvError> for ChannelError {
    fn from(err: smol::channel::TryRecvError) -> Self { ChannelError::ReceiveError(format!("TryRecv error: {}", err)) }
}
// Ensure ChannelError handles blocking RecvError
impl From<RecvError> for ChannelError {
    fn from(err: RecvError) -> Self { ChannelError::ReceiveError(format!("Blocking Recv error: {}", err)) }
}
    ...
    while config.is_none() {
        match config_rx.try_recv() {
            ...
            Err(TryRecvError::Closed) => {
                eprintln!("[WORKER] ❌ Config channel closed before receiving initial config. Shutting down.");
    ...
    loop {
        match request_rx.try_recv() {
            Ok(_) => {
                ...
                let result = get_newest_weighted_item(&current_config.server_root, &current_config.session_id.clone().unwrap());
                ...
                match result {
                    Ok(item) => {
                        ...
                        if let Err(e) = item_tx.try_send(item) {
                            println!("[WORKER] ⚠️ Failed to send response (Receiver likely dropped): {}", e);
                        }
                    }
                    Err(e) => {
...
pub fn beas_client_receiver_logic(request_tx: RequestSender, item_rx: ItemReceiver) -> Result<WeightedItem, ChannelError> {
    ...
    for _i in 1..100 { // Try for up to 10 * 50ms = 500ms
        match item_rx.try_recv() {
            ...
            Err(e) => {
                ...
                return Err(ChannelError::ReceiveError(format!("Client polling error: {}", e)));
            }
        }
    }
    
    Err(ChannelError::ReceiveError("Client response timeout.".to_string()))
}
...
pub fn start() -> (RequestSender, ItemReceiver, ConfigSender, thread::JoinHandle<Result<(), WorkerError>>) {
    ...
    (consumer_request_tx, consumer_item_rx, consumer_config_tx, worker_handle)
}
